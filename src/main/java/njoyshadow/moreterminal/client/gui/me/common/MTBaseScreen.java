package njoyshadow.moreterminal.client.gui.me.common;
...
import njoyshadow.moreterminal.client.gui.me.widget.MTVerticalButtonBar;
import njoyshadow.moreterminal.client.gui.me.widget.MTWidgetContainer;
import njoyshadow.moreterminal.client.gui.style.MTScreenStyle;
import njoyshadow.moreterminal.client.gui.style.MTSlotPosition;
...
public abstract class MTBaseScreen<T extends AEBaseMenu> extends AbstractContainerScreen<T>

{
    ...
    public MTBaseScreen(T menu, Inventory playerInventory, Component title, MTScreenStyle style) {
        ...
        if (slot instanceof CraftingTermSlot || slot instanceof  ExtendedCraftingTermSlot) {
            ...
            if (this.dbl_whichItem.isEmpty() || this.bl_clicked != slot
                ...
                for (Slot inventorySlot : slots) {
                    if (inventorySlot != null && inventorySlot.mayPickup(getPlayer()) && inventorySlot.hasItem()
                        ...
                        this.slotClicked(inventorySlot, inventorySlot.index, 0, net.minecraft.world.inventory.ClickType.QUICK_MOVE);
                    }
                }
            ...
            for (int j = 0; j < 9; ++j) {
                if (getMinecraft().options.keyHotbarSlots[j].matches(input.getValue(), -1)) {
                    ...
                    if (theSlot.getMaxStackSize() == 64) {
                        this.slotClicked(theSlot, theSlot.index, j, net.minecraft.world.inventory.ClickType.SWAP);
                        return true;
                    } else {
                        for (Slot s : slots) {
                            if (s.slot == j
        ...
        final ResourceLocation loc = new ResourceLocation(Moreterminal.MOD_ID, "textures/" + file);
        ...
        @Override
        public List<Component> getTooltipFromItem(net.minecraft.world.item.ItemStack stack) {
        ...
        protected final void switchToScreen(MTBaseScreen<?> screen) {
